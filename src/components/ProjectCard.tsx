...
export const ProjectCard: React.FC<Props> = ({
  ...
  const handleCopySql = (e: React.MouseEvent) => {
    e.stopPropagation();
    const sql = generateTargetDbSql(project.targetTable || '_heartbeat');
            ...
            {project.targetMode === 'wal_mutation' && (
              <div className="pt-2 border-t border-slate-800/60 flex items-center justify-between gap-2">
                <div className="flex items-center gap-1.5 text-slate-400 text-[11px]">
                  ...
                  <span>DDL Target ({project.targetTable || '_heartbeat'}):</span>
                </div>
                <button
                  type="button"
                  onClick={handleCopySql}
                  className="px-2.5 py-1 rounded-lg bg-emerald-500/15 border border-emerald-500/30 text-emerald-300 hover:bg-emerald-500/25 text-[11px] font-mono font-bold flex items-center gap-1 transition-all cursor-pointer active:scale-95"
                  title="Salin skrip SQL untuk target DB ini"
                >
                  {copiedSql ? <Check className="w-3 h-3 text-emerald-400" /> : <Copy className="w-3 h-3" />}
                  <span>{copiedSql ? 'Tersalin!' : 'Salin SQL Target'}</span>
                </button>
              </div>
            )}
          </div>
